...
#   coords = x y z a b c (any unique four of xyzabcuvw)
#   coefs  = 1 1 1 1 1 1 (optional, filter coefs, 0 < coef < 1, not usually reqd)
#   scales = 1 1 1 1 1 1 (optional)
...
proc connect_pins {} {
  foreach bname [lsort [array names ::XHC_WHB04B_6_BUTTONS]] {
    set thepin $::XHC_WHB04B_6_BUTTONS($bname)
...
proc wheel_setup {jogmode} {
  ...
  set ::XHC_WHB04B_6_CONFIG(coef,0) 1.0
  ...
  if [info exists ::XHC_WHB04B_6_CONFIG(coefs)] {
    ...
    foreach g $::XHC_WHB04B_6_CONFIG(coefs) {
      ...
      set ::XHC_WHB04B_6_CONFIG(coef,$idx) $g
      incr idx
    }
  }
  # defaults if not in inifile:
  set ::XHC_WHB04B_6_CONFIG(scale,0) 1.0
  ...
  if [info exists ::XHC_WHB04B_6_CONFIG(scales)] {
    ...
    foreach g $::XHC_WHB04B_6_CONFIG(scales) {
      set ::XHC_WHB04B_6_CONFIG(scale,$idx) $g
      incr idx
    }
  }

  net pendant:jog-scale      <= xhc-whb04b-6.jog.scale
  net pendant:jog-counts     <= xhc-whb04b-6.jog.counts
  ...
  #
  set idx 0
  foreach coord $::XHC_WHB04B_6_CONFIG(coords) {
    set axno $::XHC_WHB04B_6_CONFIG($coord,axno)

    setp pendant_util.coef$idx  $::XHC_WHB04B_6_CONFIG(coef,$idx)
    setp pendant_util.scale$idx $::XHC_WHB04B_6_CONFIG(scale,$idx)

    set acoord [lindex $anames $idx]
    net pendant:pos-$coord    halui.axis.$axno.pos-feedback \
                           => xhc-whb04b-6.$acoord.pos-absolute
    net pendant:pos-rel-$coord    halui.axis.$axno.pos-relative \
                               => xhc-whb04b-6.$acoord.pos-relative
    ...
    switch $jogmode {
      normal - vnormal {
        net pendant:jog-$coord    xhc-whb04b-6.jog.enable-$acoord \
                               => axis.$axno.jog-enable
      }
      plus-minus {
        # connect halui plus,minus pins
        net pendant:jog-plus-$coord     xhc-whb04b-6.jog.plus-$acoord  \
                                     => halui.jog.$axno.plus
        net pendant:jog-minus-$coord    xhc-whb04b-6.jog.minus-$acoord \
  ...
  switch $jogmode {
    normal - vnormal {
      ...
      # not used: xhc-whb04b-6.jog.velocity
      # not used: xhc-whb04b-6.jog.max-velocity
    }
    plus-minus {
      # Note: the xhc-whb04b-6 driver manages xhc-whb04b-6.jog.velocity
      net pendant:jog-max-velocity <= halui.max-velocity.value
      net pendant:jog-max-velocity => xhc-whb04b-6.jog.max-velocity
      net pendant:jog-speed        <= xhc-whb04b-6.jog.velocity
...
# begin------------------------------------------------------------------------
set ::progname "xhc-whb04b-6.tcl"
set cfg xhc-hb04b-layout2.cfg ;# default

foreach name [array names ::XHC_WHB04B_6_CONFIG] {
  set ::XHC_WHB04B_6_CONFIG($name) [string trim $::XHC_WHB04B_6_CONFIG($name) "{}"]
}

if [info exists ::XHC_WHB04B_6_CONFIG(layout)] {
  switch ${::XHC_WHB04B_6_CONFIG(layout)} {
    1 {set cfg xhc-hb04-layout1.cfg}
    2 {set cfg xhc-hb04-layout2.cfg}
    default {
      set msg "Unknown layout:<$::XHC_WHB04B_6_CONFIG(layout)>"
      set msg "$msg\ntrying: $cfg"
      popup_msg "$msg"
      # keep going
    }
...
if ![file exists $cfg] {
  ...
  set msg "$msg\nContinuing without xhc-whb04b-6"
  popup_msg "$msg"
  return ;# not an exit
}
...
if ![info exists ::XHC_WHB04B_6_CONFIG(require_pendant)] {
  set ::XHC_WHB04B_6_CONFIG(require_pendant) yes ;# default
}
set dashx -x
switch $::XHC_WHB04B_6_CONFIG(require_pendant) {
  no      {set dashx ""}
}

if [info exists ::XHC_WHB04B_6_CONFIG(sequence)] {
  set dashs "-s $::XHC_WHB04B_6_CONFIG(sequence)"
} else {
  set dashs ""
}
...
if [catch {eval $cmd} msg] {
  ...
  set msg "$msg Continuing without xhc-whb04b-6\n"
  set msg "$msg \nFailing cmd:\n$cmd"
  popup_msg "$msg"
  return ;# not an exit
}
...
#
if ![info exists ::XHC_WHB04B_6_CONFIG(jogmode)] {
  set ::XHC_WHB04B_6_CONFIG(jogmode) normal ;# default
}

set jogmode $::XHC_WHB04B_6_CONFIG(jogmode)
switch $jogmode {
  normal {}
  vnormal {}
  plus-minus {}
  default {
    set ::XHC_WHB04B_6_CONFIG(jogmode) normal
    set msg "Unkknown jogmode <$jogmode>"
    set msg "$msg  Using $::XHC_WHB04B_6_CONFIG(jogmode)"
    popup_msg "$msg"
  }
}
...
if [info exists ::XHC_WHB04B_6_CONFIG(coords)] {
  if ![is_uniq $::XHC_WHB04B_6_CONFIG(coords)] {
