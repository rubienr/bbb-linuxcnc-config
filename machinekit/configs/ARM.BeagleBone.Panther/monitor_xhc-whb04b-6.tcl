...
#         the xhc-whb04b-6.connected pin
...
proc start_monitor {} {
  ...
  if [catch {set value [hal getp xhc-whb04b-6.connected]} msg ] {
    # this can occur if loadusr xhc-whb04b-6 fails, 
    puts "$::progname: xhc-whb04b-6 comp not loaded" 
    puts "$::progname: <$msg>"
    puts "$::progname: exiting"
    return
  }
  set ::connected false
  if [catch { set ::connected [hal getp xhc-whb04b-6.connected]
            } msg] {
  ...
  if [catch { set ::require_pendant [hal getp xhc-whb04b-6.require_pendant]
            } msg] {
...
proc check {} {
  if [catch { set ::connected_new [hal getp xhc-whb04b-6.connected]
            } msg] {
    puts "$::progname <$msg>"
  }
  ...
  if {$::connected_new != $::connected} {
    ...
    if $::require_pendant {
      if {$::connected_new} {
        ...
        # xhc-whb04b-6 will timeout and exit so no more checks
...
# begin -----------------------------------------------------
if ![info exists ::env(monitor_xhc-whb04b-6)] {
  set ::env(monitor_xhc-whb04b-6) $::argv0
  exec ./monitor_xhc-whb04b-6.tcl & ;# restart this with env(monitor_xhc-whb04b-6) set
  exit 0
} else {
